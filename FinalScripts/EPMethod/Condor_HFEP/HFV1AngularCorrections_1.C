...
Int_t HFV1AngularCorrections_1(){
  Initialize();
  FillPTStats();
  AngularCorrections();
  return 0;
...
void Initialize(){
  ...
  //Calo Tower Tree
  chain->Add("/hadoop/store/user/jgomez2/ForwardTrees/2010/PanicTime/ForwardTrees_skim_alltracks_2010_112_2_dCP.root");
  //Tracks Tree
  chain2->Add("/hadoop/store/user/jgomez2/ForwardTrees/2010/PanicTime/ForwardTrees_skim_alltracks_2010_112_2_dCP.root");

  NumberOfEvents = chain2->GetEntries();
  //Create the output ROOT file
  myFile = new TFile("HFEP_AngularCorrections_1.root","recreate");

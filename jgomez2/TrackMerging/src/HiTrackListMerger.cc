...
//    using namespace reco;
    ...
    // this allows HiTrackListMerger to be used as a cleaner only if handed just one list
    ...
    if (trackCollection1.isValid()) {
      ...
      TC1 = &s_empty1;
      edm::LogWarning("HiTrackListMerger") << "1st TrackCollection " << trackProducer1 << " not found; will only clean 2nd TrackCollection " << trackProducer2 ;
    }
    ...
    if (trackCollection2.isValid()) {
      ...
      //std::cout << "2nd collection " << trackProducer2 << " has "<< TC2->size() << " tracks" << std::endl ;
        ...
        TC2 = &s_empty2;
        edm::LogWarning("HiTrackListMerger") << "2nd TrackCollection " << trackProducer2 << " not found; will only clean 1st TrackCollection " << trackProducer1 ;
    }
  ...
  // 
   ...
   if ( 0<tC1.size() ){
      ...
      for (reco::TrackCollection::const_iterator track=tC1.begin(); track!=tC1.end(); track++){
        ...
        if ((short unsigned)track->ndof() < 1){
          selected1[i]=0; 
          //std::cout << "L1Track "<< i << " rejected in HiTrackListMerger; ndof() < 1" << std::endl ;
          continue;
        }
        if (track->normalizedChi2() > maxNormalizedChisq){
          selected1[i]=0; 
          //std::cout << "L1Track "<< i << " rejected in HiTrackListMerger; normalizedChi2() > maxNormalizedChisq " << track->normalizedChi2() << " " << maxNormalizedChisq << std::endl ;
          continue;
        }
        if (track->found() < minFound){
          selected1[i]=0; 
          //std::cout << "L1Track "<< i << " rejected in HiTrackListMerger; found() < minFound " << track->found() << " " << minFound << std::endl ;
          continue;
        }
        if (track->pt() < tC1MinPt || track->pt() > tC1MaxPt){
          selected1[i]=0; 
          //std::cout << "L1Track "<< i << " rejected in HiTrackListMerger; pt() < minPT " << track->pt() << " " << minPT << std::endl ;
          continue;
        }
   ...
   if ( 0<tC2.size() ){
      ...
      for (reco::TrackCollection::const_iterator track=tC2.begin(); track!=tC2.end(); track++){
        ...
        if ((short unsigned)track->ndof() < 1){
          selected2[i]=0; 
          //std::cout << "L2Track "<< i << " rejected in HiTrackListMerger; ndof() < 1" << std::endl ;
          continue;
        }
        if (track->normalizedChi2() > maxNormalizedChisq){
          selected2[i]=0; 
          //std::cout << "L2Track "<< i << " rejected in HiTrackListMerger; normalizedChi2() > maxNormalizedChisq " << track->normalizedChi2() << " " << maxNormalizedChisq << std::endl ;
          continue;
        }
        if (track->found() < minFound){
          selected2[i]=0; 
          //std::cout << "L2Track "<< i << " rejected in HiTrackListMerger; found() < minFound " << track->found() << " " << minFound << std::endl ;
          continue;
        }
        if (track->pt() < tC2MinPt || track->pt() > tC2MaxPt){
          selected2[i]=0; 
          //std::cout << "L2Track "<< i << " rejected in HiTrackListMerger; pt() < minPT " << track->pt() << " " << minPT << std::endl ;
          continue;
        }
      }//end loop over tracks
   }//end more than 0 track
